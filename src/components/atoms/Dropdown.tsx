  ...
  return (
    <Combobox value={selectedItem} onChange={setSelectedItem}>
      <div className={`relative w-64 ${comboboxClassName}`}>
        {/* Combobox Input */}
        <div
          className="relative w-full flex items-center space-x-2 border border-[#008485] rounded-md px-3 py-2 bg-white">
          <Combobox.Input
          ...
          />
          <Combobox.Button>
            <FiChevronDown className="w-5 h-5 text-[#008485]" aria-hidden="true" />
          </Combobox.Button>
        </div>
        ...
        {filteredItems.length > 0 && (
          <Combobox.Options
            className="absolute w-full mt-2 max-h-60 overflow-auto bg-white border border-gray-300 rounded-md shadow-lg z-10">
